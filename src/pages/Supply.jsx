        ...
        <div className="p-8 max-w-7xl mx-auto min-h-screen text-slate-100 relative">
            ...
            {showReceiptModal && currentReceipt && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in zoom-in-95 duration-200">
                    <div className="bg-slate-900 border border-slate-700 text-white w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-none shadow-2xl relative flex flex-col">

                        {/* Paper Header */}
                        <div className="p-8 border-b-2 border-dashed border-slate-700 bg-slate-950 flex justify-between items-start">
                            <div>
                                <h1 className="text-2xl font-black tracking-tighter text-white">LINO LAB</h1>
                                <p className="text-xs uppercase tracking-widest text-slate-400 mt-1">Recibo de Entrada de Inventario</p>
                            </div>
                            <div className="text-right">
                                <div className="bg-emerald-900/30 text-emerald-400 px-3 py-1 rounded-full text-xs font-bold border border-emerald-800 inline-block mb-2">
                                    COMPLETADO
                                </div>
                                <p className="text-sm font-mono text-slate-400">{formatDate(currentReceipt.fecha_entrada)}</p>
                            </div>
                        </div>
                        ...
                        <div className="p-8 bg-slate-900 flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-8 mb-8">
                                <div>
                                    <span className="block text-[10px] uppercase text-slate-500 font-bold tracking-wider">Proveedor</span>
                                    <span className="block text-lg font-bold text-slate-200 border-b border-slate-800 pb-1">
                                        {currentReceipt.empresarios?.nombre_empresario}
                                    </span>
                                </div>
                                <div>
                                    <span className="block text-[10px] uppercase text-slate-500 font-bold tracking-wider">Referencia / Folio</span>
                                    <span className="block text-lg font-mono text-slate-300 border-b border-slate-800 pb-1">
                            ...
                            <table className="w-full text-sm mb-8">
                                <thead>
                                    <tr className="text-xs uppercase text-slate-300 border-b-2 border-slate-700">
                                ...
                                <tbody className="font-mono text-slate-300">
                                    {currentReceipt.details?.map((detail, idx) => (
                                        <tr key={idx} className="border-b border-slate-800">
                                            <td className="py-3">
                                                <div className="font-bold text-slate-200">{detail.productos?.nombre_producto}</div>
                                                <div className="text-[10px] text-slate-500">{detail.productos?.sku_producto}</div>
                                            </td>
                                            <td className="py-3 text-center text-slate-200">{detail.cantidad_ingresada || detail.cantidad}</td>
                                            <td className="py-3 text-right text-slate-200">{formatCurrency(detail.costo_unitario_ingreso || detail.costo_unitario)}</td>
                            ...
                            {/* Totals */}
                            <div className="flex justify-end border-t-2 border-slate-700 pt-4">
                                <div className="text-right">
                        ...
                        {/* Actions Footer */}
                        <div className="p-6 bg-slate-950 border-t border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between">
                            <div className="flex items-center gap-2 w-full md:w-auto">
                                <input
                                    type="email"
                                    placeholder="correo@ejemplo.com"
                                    className="px-4 py-2 border border-slate-600 bg-slate-800 rounded-lg text-sm w-full outline-none focus:border-blue-500 text-white placeholder-slate-500"
                                    id="emailInput"
                                />
                                <button
                                    onClick={() => handleSendEmail(document.getElementById('emailInput').value)}
                                    className="bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2"
                                >
                            ...
                            <div className="flex gap-3 w-full md:w-auto">
                                <button
                                    ...
                                    className="flex-1 md:flex-none bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20"
                                >
                                    <Download size={18} /> Descargar PDF
                                </button>
                                <button
                                    onClick={() => setShowReceiptModal(false)}
                                    className="px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg font-bold transition-colors"
                                >
                                    Cerrar
                                </button>
                            </div>
